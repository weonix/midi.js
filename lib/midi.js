...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
	  ...
	  player.scheduleLoop = function () {
	    ...
	    loopHandler = setInterval(function () {
	      ...
	      for (var _note in noteOffRegistrar) {
	        ...
	        // }
	        var noteOff = noteOffRegistrar[_note];
	  ...
	  _root2.default.getActiveNotes = function () {
	    var currentTime = player.getAudioContextPlaytime() * 1000;
	    var notes = [];
	
	    for (var noteOn in noteRegistrar) {
	      var noteOnData = noteRegistrar[noteOn];
	      if (noteOnData.now <= currentTime) {
	        var alreadyOff = false;
	        for (var noteOff in noteOffRegistrar) {
	          var noteOffData = noteOffRegistrar[noteOff];
...
/***/ function(module, exports) {
	...
	function Replayer(midiFile, timeWarp, eventProcessor, bpm, timeSignitures) {
	  var trackStates = [];
	    ...
	    try {
	      for (var _iterator = timeSignitures[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	        ...
	        if ((currentTick - sign.time) % beatTime != 0) {
	          wait = beatTime - (currentTick - sign.time) % beatTime;
	        } else {
	          wait = 0;
	        }
