...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	(function () {
	  ...
	  var scheduleTracking = function scheduleTracking(channel, note, currentTime, wait, message, velocity, eventObj) {
	    var wait = wait;
	    return setTimeout(function () {
	      var data = {
	        channel: channel,
	      ...
	      } else if (player.getAudioContextPlaytime() >= queuedTime / 1000 - player.minLookAheadTime && queuedTime < player.endTime) {
	        // grab next sequence
	        startAudio(queuedTime, true);
	      }
	    }, wait);
	      ...
	      switch (event.subtype) {
	        ...
	        case 'noteOn':
	          ...
	          eventQueue.push({
	            event: event,
	            time: queueTime,
	            source: _root2.default.noteOn(event, channelId, note, event.velocity, delay),
	        ...
	        case 'noteOff':
	          ...
	          eventQueue.push({
	            event: event,
	            time: queueTime,
	            source: _root2.default.noteOff(event, channelId, note, delay),
	  ...
	  var stopAudio = function stopAudio() {
	    ...
	    for (var key in noteRegistrar) {
	      ...
	      if (noteRegistrar[key].message === 144 && onMidiEvent) {
	        var endPlaybackMidiData = [128, _o.note, _o.velocity];
	        onMidiEvent({
	          channel: _o.channel,
	          note: _o.note,
	          now: _o.now,
	    ...
	    if (_root2.default.stopAllNotes) {
	      _root2.default.stopAllNotes(player.minLookAheadTime);
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
	  ...
	  midi.noteOn = function (event, channel, note, velocity, delay) {
	    output.send(event.rawData, delay * 1000);
	    onNoteList[note * 100 + channel] = { note: note, channel: channel };
	    //output.send([0x90 + channel, note, velocity], delay * 1000)
	  };
	
	  midi.noteOff = function (event, channel, note, delay) {
	    output.send(event.rawData, delay * 1000);
	    delete onNoteList[note * 100 + channel];
	  ...
	  midi.stopAllNotes = function (lookAhead) {
	    ...
	    for (var iterator in onNoteList) {
	      var note = onNoteList[iterator];
	      output.send([0xB0 + note.channel, note.note, 0]);
	      output.send([0xB0 + note.channel, note.note, 0], window.performance.now() + lookAhead);
	    }
	    onNoteList = {};
	  ...
	  midi.connect = function (opts) {
	    ...
	    navigator.requestMIDIAccess().then(function (access) {
	      ...
	      if (typeof pluginOutputs === 'function') {
	        ...
	        try {
	          for (var _iterator = pluginOutputs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	            var out = _step.value;
