...
(function () {
  ...
  var scheduleTracking = function (channel, note, currentTime, wait, message, velocity, eventObj) {
    ...
    return setTimeout(function () {
      ...
      } else if (player.getAudioContextPlaytime() >= (queuedTime / 1000) - 0.1 && queuedTime < player.endTime) { // grab next sequence
        startAudio(queuedTime, true)
      }
    }, wait)
  }
  ...
  var startAudio = function (currentTime, fromCache, onsuccess) {
    ...
    for (var n = 0; n < length && messages < 5; n++) {
      ...
      var scheduelWait = delay - ctx.currentTime / 1000;
      ...
      switch (event.subtype) {
        ...
        case 'noteOn':
          ...
          eventQueue.push({
            ...
            interval: scheduleTracking(event, channelId, note, queuedTime + player.startDelay, scheduelWait, 144, event.velocity, event)
          })
          messages++
          break
        case 'noteOff':
          ...
          eventQueue.push({
            ...
            interval: scheduleTracking(event, channelId, note, queuedTime, scheduelWait, 128, 0, event)
          })
          break
        default:
          break
