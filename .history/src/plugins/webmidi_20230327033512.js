...
(function () {
  ...
  midi.send = function (data, delay) { // set channel volume
    output.send(data, window.performance.now() + delay * 1000)
  }

  midi.setController = function (event, channel, type, value, delay) {
    
    output.send(event.rawData, window.performance.now() + delay * 1000)
    //output.send([channel, type, value], delay * 1000)
  }

  midi.setVolume = function (channel, volume, delay) { // set channel volume
    output.send([0xB0 + channel, 0x07, volume], window.performance.now() + delay * 1000)
  }
