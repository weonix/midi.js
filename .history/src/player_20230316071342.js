...
(function () {
  ...
  player.setAnimation = function (callback) {
    ...
    var frame = function () {
      ...
      // /
			if(currentTime == 0 && midi.playing)
       currentTime = ((Date.now() - midi.ctxStartTime * 10) - midi.playingStartTime) / 100 * MIDI.Player.BPM;
          var percent = currentTime / endTime;			if(midi.lastCallbackTime!=currentTime){
          var total = currentTime / 1000;				var endTime = midi.endTime;
          var minutes = total / 60;				//var percent = currentTime / endTime;
          var seconds = total - (minutes * 60);				var t1 = currentTime / 1000;
          var t1 = minutes * 60 + seconds;				var t2 = endTime / 1000;
          var t2 = (endTime / 1000);				///
          ///				if (t2 - t1 < -1.0) {
          if (t2 - t1 < -1.0) {					return;
            return;				} else {
